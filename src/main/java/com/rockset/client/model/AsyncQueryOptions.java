...
public class AsyncQueryOptions {
  ...
  /**
   * If the query completes before the client timeout, the results are returned. Otherwise if the
   * client timeout is exceeded, the query id will be returned, and the query will continue to run
   * in the background for up to 30 minutes. (The 30 minute timeout can be configured lower with
   * timeout_ms.) &#x60;async_options.client_timeout_ms&#x60; only applies when &#x60;async&#x60; is
   * true. The default value of &#x60;client_timeout_ms&#x60; is 0, so async query requests will
   * immediately return with a query id by default.
   *
  ...
  /**
   * [DEPRECATED] Use the query request &#x60;max_initial_results&#x60; instead. The maximum number
   * of results you will receive as a client. If the query exceeds this limit, the remaining results
   * can be requested using a returned pagination cursor. In addition, there is a maximum response
   ...
   * that the system will attempt to complete query execution before aborting the query and
   * returning an error. This must be set to a value that is greater than or equal to the client
   * timeout, and the maximum value of this timeout is 30 minutes.
   *
   * @return timeoutMs
   */
